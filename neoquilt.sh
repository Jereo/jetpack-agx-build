        ...
        if [ "${isFile}" == "true" ]; then
            ...
            if [ "x${addedFiles}" != "x" ]; then
                ...
                do
                    ...
                    quilt add ${addedFile} > /dev/null 2>&1
                done
            fi
...
function GetQuiltRoot(){
    ...
    dps=`echo ${curDir} | sed -r "s/\// /g" | wc -w`
    res=`expr "${curDir##*/}" : '^Linux_for_Tegra$'`
    if [ ${dps} -lt 4 -o ${res} -eq 15 ]; then
...
function GetRootFSDir(){
    ...
    if [ "x$dirs" == "x" ];then
        local dirs=`find . -maxdepth 8 -path "*/rootfs/*" -prune -o -path "*sources" -prune -o -path "*u-boot" -prune -o \
                ...
                -path "*tegra_multimedia_api" -prune -o -path "*_installer" -prune -o -path "*jetpack_docs" -prune -o \
                -path "*jetpack_download" -prune -o -path "*NVIDIA_Tegra_System_Profiler" -prune -o -path "*nvl4t_docs" -prune -o \
                -path "*tmp" -prune -o -type l -name "rootfs" -print | sed "s/\.\///g"`
    fi
...
function GetRootPatchDir(){
    ...
    local dirs=`find . -maxdepth 8 -path "*/rootfs" -prune -o -path "*u-boot" -prune -o -path "*images" -prune -o \
     ...
     -path "*NVIDIA_Tegra_System_Profiler" -prune -o -path "*nvl4t_docs" -prune -o -path "*tmp" -prune -o -path "*pc" -prune -o \
...
function RecoverPatches(){
    GetQuiltRoot
    workPath=`echo ${quiltRoot%/*}`
    ResetWorkplace
    quilt push -a
    result=$?
    if [ ${result} -eq 2 ]; then
        neoLog "SUCCEED" "patches fully applied."
        return
    fi
    if [ ${result} -ne 0 ]; then
        neoLog "" "reset workplace."
        ResetWorkplace
        quilt push -a
    fi
    checkWorkPath
    commitInfo=`cat ${quiltRoot}/series`
    for gitP in ${gitPaths}
    do
        cd ${gitP} > /dev/null
        git add . > /dev/null
        git commit -m "${commitInfo}" > /dev/null
        cd - > /dev/null
    done
    GetRootFSDir
    GetRootPatchDir
    ApplyRootFsPatches
    neoLog "SUCCEED" "apply patches succeed."
}

function ExecBlackList(){
    if [ ! -e ${rootPatchDir}/${blackListName} ];then
        return
    fi
    while  read LINE
    do
...
function DeleteRootFsPatches(){
    rm -rvf ${quiltRoot%/*}/.pc
    if [ "X${rootPatchDir}" != "X" ]; then
        rm -rf ${rootPatchDir} ${quiltRoot}/../.pc
        if [ "X${rootFsDir}" == "X" ]; then
            neoLog "ERROR" "couldn't find rootfs directory."
            exit 1
        fi
